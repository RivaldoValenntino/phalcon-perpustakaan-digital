...
<ul class="submenu-inner">
  {%- for menu in menuList -%} {%- set toggleClass = menu.has_children ?
  'dropdown-toggle' : '' -%} {%- set href = menu.link_menu is not empty and not
  menu.has_children ? url() ~ 'panel/' ~ menu.link_menu : 'javascript:;' -%}
  <li class="nav-item" data-menu_id="{{ menu.id_menu }}">
    <a class="nav-link {{ toggleClass }}" href="{{ href }}">
      ...
      <span
        class="badge badge-danger py-1 radius-round text-90 mr-0 badge-sm"
        id="{{ menu.badge_id }}"
      ></span>
      <?php endif; ?>
...
<nav class="pt-3" aria-label="Main">
  <ul class="nav flex-column has-active-border">
    {% for menu in menuModel.getUserMenuList() %} {% set toggleClass =
    menu.link_menu is not empty ? '' : 'dropdown-toggle' %} {% set href =
    menu.link_menu ? url() ~ 'panel/' ~ menu.link_menu : 'javascript:;' %}
    <li class="nav-item" data-menu_id="{{ menu.id_menu }}">
